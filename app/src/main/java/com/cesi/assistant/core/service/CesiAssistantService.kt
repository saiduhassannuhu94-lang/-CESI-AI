...
class CesiAssistantService : Service() {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            getSystemService(NotificationManager::class.java).createNotificationChannel(
                NotificationChannel(CHANNEL_ID, "CESI Assistant", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    private fun showOverlay() {
        ...
        container.addView(orb, LinearLayout.LayoutParams(76, 76))
        container.addView(statusText, LinearLayout.LayoutParams(110, 40))
    ...
    private fun startListening() {
        ...
        speechRecognizer?.setRecognitionListener(object : RecognitionListener {
            override fun onReadyForSpeech(p: Bundle?) { isListening = true; setStatus("Listening") }
            override fun onBeginningOfSpeech() { isListening = true; setStatus("Listening") }
            override fun onEndOfSpeech() { isListening = false; setStatus("Processing") }
            override fun onError(e: Int) { isListening = false; setStatus("Ready") }
            override fun onResults(results: Bundle?) {
                isListening = false
                val text = results?.getStringArrayList(SpeechRecognizer.RESULTS_RECOGNITION)?.firstOrNull().orEmpty()
                if (text.isBlank()) setStatus("Ready") else handleCommand(text)
            }
        ...
        try { speechRecognizer?.startListening(intent) } catch (_: Exception) { isListening = false }
    }
    ...
    private fun speak(text: String) {
        ...
        mainHandler.postDelayed({ setStatus("Ready") }, 1800)
    }

    private fun setStatus(text: String) {
        mainHandler.post {
            statusText?.text = text
            statusText?.visibility = if (text == "Ready") View.GONE else View.VISIBLE
        }
    }
